...
parser.add_argument('--trained_model', default='weights_1e-5/ssd300_COCO_10000.pth',
...
def test_net(save_folder, net, cuda, testset, transform, thresh, draw=True):
    ...
    filename = save_folder + 'test_200_0.001.txt'
    ...
    for i in range(num_images):
        ...
        #         cv2.rectangle(img, (int(annotation[k][0]), int(annotation[k][1])),
        #                       (int(annotation[k][2]), int(annotation[k][3])), (0, 255, 0), 2)
        #
        # # cv2.imshow('gt&pre', img)
        # cv2.imwrite('*/SSD/Results/' + str(img_id) + '.jpg', img)
        # # cv2.waitKey(500)
